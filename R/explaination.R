...
#' Logistic regression
#'
#' A dataset containing credit and loan default information for binary and binomial logistic regression.
#'
...
#' }
#'
#' @note Use `defaulted` for binary logistic regression, and `successes` and `trials` for binomial logistic regression.
"Logistic_Regression"
...
#' Ordinal Logistic Regression Data with Non-Proportional Odds
#'
...
#' \enumerate{
#'   \item Fit a proportional odds model using \code{MASS::polr()}
#'   \item Test the proportional odds assumption using the Brant test or
#'         by comparing separate binary logistic regressions at each cutpoint
#'   \item If the assumption is violated, consider using \code{VGAM::vglm()}
#'         with \code{family = cumulative(parallel = FALSE)} for a full
